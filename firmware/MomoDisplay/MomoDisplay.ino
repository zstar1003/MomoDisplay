...
#define DASHBOARD_DATE_SCALE 2
#define DASHBOARD_DATE_PADDING_X 2
#define DASHBOARD_DATE_PADDING_Y 5
#define DASHBOARD_DATE_X 22
#define DASHBOARD_DATE_Y 21
#define DASHBOARD_HEADER_LINE_Y 80
...
static uint8_t *captureUTF8Bitmap(const char *text, const uint8_t *font, int paddingX, int paddingY, int &width, int &height)
{
  u8g2->clearBuffer();
  u8g2->setDrawColor(1);
  u8g2->setFont(font);
  ...
  width = u8g2->getUTF8Width(text) + paddingX * 2;
  ...
  u8g2->drawUTF8(paddingX, paddingY + ascent, text);

  size_t bitmapSize = (size_t)width * (size_t)height;
  ...
  for (int y = 0; y < height; y++) {
    for (int x = 0; x < width; x++) {
      bitmap[(size_t)y * width + x] = logicalBufferPixel(x, y) ? 1 : 0;
    }
  }
...
static void drawScaledBitmapAt(const uint8_t *bitmap, int sourceWidth, int sourceHeight, int destX, int destY, int scale)
{
  if (bitmap == nullptr || sourceWidth <= 0 || sourceHeight <= 0 || scale <= 0) {
    return;
  }
...
static void drawDefaultPage()
{
  ...
  char dateText[32];
  char timeText[8];
  static const char yearText[] = "\xe5\xb9\xb4";
  static const char monthText[] = "\xe6\x9c\x88";
  static const char dayText[] = "\xe6\x97\xa5";

  snprintf(dateText, sizeof(dateText), "%04u%s%u%s%u%s", now.year, yearText, now.month, monthText, now.day, dayText);
  snprintf(timeText, sizeof(timeText), "%02u:%02u", now.hour, now.minute);

  int dateBitmapWidth = 0;
  int dateBitmapHeight = 0;
  uint8_t *dateBitmap = captureUTF8Bitmap(
    dateText,
    u8g2_font_wqy16_t_gb2312,
    DASHBOARD_DATE_PADDING_X,
    DASHBOARD_DATE_PADDING_Y,
    dateBitmapWidth,
    dateBitmapHeight
  );
  ...
  u8g2->drawHLine(24, DASHBOARD_HEADER_LINE_Y, 352);

  drawScaledBitmapAt(dateBitmap, dateBitmapWidth, dateBitmapHeight, DASHBOARD_DATE_X, DASHBOARD_DATE_Y, DASHBOARD_DATE_SCALE);
  free(dateBitmap);
